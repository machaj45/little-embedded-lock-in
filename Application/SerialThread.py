...
class SerialThread(QtCore.QThread):
    ...
    def initrun(self):
        self.ser = serial.Serial(baudrate = self.baudrate, bytesize = serial.EIGHTBITS, parity = serial.PARITY_NONE, stopbits = serial.STOPBITS_ONE,timeout = SER_TIMEOUT,write_timeout=0.01)
        self.test = 0
        self.count = 1
    ...
    def scan(self):
        ...
        return self.portConnected
    def setGui1(self):
        self.gui.label15.setText("Amplitude - {0} %, Upp = {1} mV".format( self.gui.text[4][:-2],int(3300*(float(self.gui.text[4][:-2])/100))))
    ...
    def setGui2(self):
        self.gui.label25.setText("Amplitude - {0} %, Upp = {1} mV".format( self.gui.text[4][:-2],int(3300*(float(self.gui.text[4][:-2])/100))))
            ...
            if self.data:
                if len(self.data)==8:
                    ...
                    if(math.sqrt((self.gui.dataX)**2+ (self.gui.dataY)**2)>0):
                        ...
                        self.gui.texttoupdate="X = " + str(self.gui.dataX)+" Y = "+str(self.gui.dataY) + " theta = "+str(theta) +"°, R ="+ str(RR)+'dB'
                    else:
                        RR = '-inf'
                        self.gui.texttoupdate="X = " + str(self.gui.dataX)+" Y = "+str(self.gui.dataY) + " theta = "+str(theta) +"°, R ="+ str(RR)+' dB'
                    self.gui.databin=False
